...
{
    public byte[] GenerarReporte(List<ResultadoComparacion> resultados)
    {
        using var workbook = new XLWorkbook();

        var hoja = workbook.Worksheets.Add("Estudiantes Repetidos");

        // ===== TÍTULO =====
        hoja.Cell("A1").Value = "REPORTE DE ESTUDIANTES REPETIDOS";

        // Ahora son 6 columnas: A hasta F
        ...
        hoja.Cell(filaEncabezado, 1).Value = "Documento";
        hoja.Cell(filaEncabezado, 2).Value = "Nombre";
        hoja.Cell(filaEncabezado, 3).Value = "Cantidad";
        hoja.Cell(filaEncabezado, 4).Value = "Programas / Áreas";
        hoja.Cell(filaEncabezado, 5).Value = "Instituciones";
        hoja.Cell(filaEncabezado, 6).Value = "Archivos";
        ...
        foreach (var resultado in resultados)
        {
            ...
            hoja.Cell(fila, 4).Value =
                string.Join(", ", resultado.Programas);

            hoja.Cell(fila, 5).Value =
                string.Join(", ", resultado.Instituciones);

            hoja.Cell(fila, 6).Value =
                string.Join(", ", resultado.Archivos);
